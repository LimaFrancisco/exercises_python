...
print(f"Projection of Allowance Spending\n{"=" * 32}\n")

while value != 0:

    try:
        value = float(input("enter the employee's salary(0=end): "))
    except ValueError:
        print("invalid value")
        continue
    ...
    if value < 1000 and value > 0:
        allowance.append(float(100))
        miniAmount += 1
    else:
        allowance.append(value * 0.2)

print("\nsalary      - allowance")
for index in range(0, len(salary)):
    print(f"R$ {salary[index]:>8.2f} - R$ {allowance[index]:>6.2f}")

print(f"\n{len(salary):.0f} employee(s) were sued")
print(f"total spent on allowances: R$ {sum(allowance):.2f}")
print(f"minimum amount paid to {miniAmount:.0f} employees")